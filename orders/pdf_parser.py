...
# Слова-компоненты/характеристики двери: если скобки содержат такое —
# это состав комплекта или описание, а не помещение.
# Пример: «Комплект Secret 59 (Полотно. Короб. Петли Anselmi. Замок WC)».
_COMPONENT_WORDS = (
    'полотно', 'короб', 'петл', 'замок', 'ручк', 'кромк', 'грунт',
    'наличник', 'добор', 'механизм', 'стекл', 'фурнитур', 'накладк',
    'створч', 'скидк', 'под заказ', 'анодир', 'обычное', 'левое', 'правое',
    'реверс', 'открыван', ' мм', 'по запросу', 'крашен', 'в цвет', 'эмал',
    'выбрать', 'каталог',
)
...
def _extract_room_from_description(desc: str) -> str:
    ...
    Помещение пишут в КОНЦЕ описания, а в начале могут быть скобки с
    характеристиками («(Цвет ЭМАЛИ выбрать) (Спальня 1)») — поэтому кандидатов
    перебираем с конца.
    """
    if not desc:
        return ''
    matches = reversed(re.findall(r'\(([^()]+)\)', desc))
    for m in matches:
        text = m.strip()
        ...
        if re.match(r'^\d', text):
            continue
        lowered = text.lower()
        if 'нет в замере' in lowered:
            continue
        # Состав комплекта («Полотно. Короб. Петли…») — не помещение
        if any(w in lowered for w in _COMPONENT_WORDS):
            continue
        # Помещения пишут кириллицей («Спальня2»); чисто латиница/цифры —
        # это цвета/артикулы («Bronze, Grey, Clear»)
...
def _parse_door_row(joined_line: str) -> Optional[Dict[str, Any]]:
    ...
    # ---- Нормализация новых КП (2026): рек.проём указан тройкой «Ш*В*Глубина» ----
    # Пример: '... 1 2000*800 900*2070*165' — полотно 2000*800, проём 900*2070, глубина 165.
    # Иногда звёздочка перед глубиной теряется и она слипается с высотой: '900*2070165'.
    # Без нормализации хвост «900*2070*165» ловился панельной стратегией
    # «<qty> * <H>*<W>» и qty становился 900 → КП раскрывалось в сотни позиций.
    # Уточнение полотна в скобках ('2000(2010)*800') убираем, чтобы размер читался.
    head = re.sub(r'(\d{3,4})\s*\(\d+\)\s*\*', r'\1*', head)
    m_triple = re.search(
        r'(\d{3,4}\s*\*\s*\d{2,4})\s+(\d{3,4})\s*\*\s*([12]\d{3})(?:\s*\*\s*\d{2,4}|(\d{2,4}))(?=\s|$)',
        head,
    )
    if m_triple:
        head = (
            head[:m_triple.start()]
            + f'{m_triple.group(1)} {m_triple.group(2)}*{m_triple.group(3)}'
            + head[m_triple.end():]
        )
    ...
    try:
        with pdfplumber.open(pdf_file) as pdf:
            ...
            for anchor, cont in _split_section_into_rows(sections.get('doors', '')):
                ...
                # Страховка от мисраспознавания: Кол-во в реальных КП — единицы,
                # десятки максимум. Если распозналось больше — это почти наверняка
                # кусок размера, попавший в qty; не размножаем КП в сотни позиций.
                if qty > 50:
                    logger.warning('parse_kp_pdf: подозрительное qty=%d в строке %r — считаем 1', qty, anchor[:120])
                    qty = 1
