...
## 📦 Дополнительные рекламации (complaints_loaddata.json, 5 штук)

Дополнительный набор из 5 новых рекламаций для loaddata:

```bash
# Сначала загрузите зависимости (если ещё не загружены):
python manage.py loaddata users/fixtures/test_cities.json users/fixtures/test_users.json projects/fixtures/test_production_sites.json projects/fixtures/initial_complaint_reasons.json

# Затем загрузите 5 рекламаций:
python manage.py loaddata projects/fixtures/complaints_loaddata.json
```

Рекламации: **ORD-2025-101** — ORD-2025-105 (транспортный бой, ошибка монтажа, ошибка замера, фурнитура брак, производственный дефект)
...
PushSubscription.objects.filter(user=user, is_active=False).delete()

from projects.models import Notification
notif = Notification.objects.filter(recipient=user).order_by('-created_at').first()
print(notif.notification_type, notif.is_sent, notif.sent_at)
