...
# Подписи соседних полей шапки: на них значение адреса заканчивается.
_ADDRESS_STOP = (
    r'телефон|тел\.|email|e-mail|комментарий|модель\s+полотна'
    r'|подразделение|менеджер|покупатель|дата|срок\s+готовности'
)

_ADDRESS_PATTERNS = (
    # «Адрес доставки Каратау 18», «Адрес: ул. Такая-то, 5».
    # Подпись поля («доставки», «установки») съедаем явно: раньше она
    # затекала в значение, и в заказ уезжало «доставки Каратау 18».
    # Отрицательный просмотр в начале — на случай пустого адреса: иначе
    # значением становилась подпись следующего поля вместе с телефоном.
    re.compile(
        r'адрес[а]?\s*(?:доставки|установки|монтажа|объекта)?\s*:?\s*'
        r'(?!(?:' + _ADDRESS_STOP + r'))(.+?)\s*(?=(?:' + _ADDRESS_STOP + r')|$)',
        re.IGNORECASE | re.DOTALL,
    ),
    # Адреса доставки в КП нет вовсе — берём адрес подразделения:
    # «Подразделение г. Казань, ул. Ямашева д 93, ТК САВИНОВО, тел ...».
    re.compile(
...
def _extract_address(text: str) -> str:
    """Адрес доставки из шапки КП."""
    for pattern in _ADDRESS_PATTERNS:
        match = pattern.search(text or '')
        if not match:
            continue
        address = re.sub(r'\s+', ' ', match.group(1)).strip()
        # Хвосты соседних полей, если они прилипли без разделителя.
        address = re.sub(
            r'\s*,?\s*(тел|телефон|email|e-mail)\b.*$', '', address, flags=re.IGNORECASE,
        )
